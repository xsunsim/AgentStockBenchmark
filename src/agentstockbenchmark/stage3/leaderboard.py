...
from agentstockbenchmark.io import atomic_write_csv, atomic_write_text
from agentstockbenchmark.settings import DEFAULT_RESULTS_REPO


def build_leaderboard(results_repo: Path = DEFAULT_RESULTS_REPO) -> dict[str, Path]:
    ...
    atomic_write_text(html_path, render_leaderboard_html(df))

    return {"csv": csv_path, "markdown": md_path, "html": html_path}


def render_leaderboard_html(df: pd.DataFrame) -> str:
    rows = []
...
<head>
  <meta charset="utf-8">
  <title>AgentStockBenchmark Leaderboard</title>
  <style>
...
<body>
  <h1>AgentStockBenchmark Leaderboard</h1>
  <table>
    <thead>
